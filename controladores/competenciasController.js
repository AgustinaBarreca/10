...
function listarGeneros(req, res) {
  ...
  con.query(sql, function(error, resultado, campos) {
    if(error) {
      console.log("Hubo un error para cargar los Generos.", error.message);
...
function buscarPeliculas(genero, director, actor) {
  var unionBd = function() {
    ...
    if(director != undefined) {
      sql = sql + (` JOIN director_pelicula ON pelicula.id = director_pelicula.pelicula_id
               JOIN director ON director_pelicula.director_id = director.id`)
    }
    if(actor != undefined) {
      sql = sql + (` JOIN actor_pelicula ON pelicula.id = actor_pelicula.pelicula_id
  ...
  var filtradoBd = function() {
    ...
    if(genero > 0) {
      sql = sql.concat('genero_id = ' + genero);
    }
    if(genero > 0 && (director > 0 || actor > 0)) {
      sql = sql.concat(' AND ');
    }
    if(director > 0) {
      sql = sql.concat('director.id = ' + director);
    }
    if(director > 0 && actor > 0) {
      sql = sql.concat(' AND ');
    }
    if(actor > 0) {
      sql = sql.concat( 'actor.id = ' + actor);
    }
    return sql;
  }
...
function obtener1Competencia(req, res) {
  ...
  var sql = 'SELECT * FROM vs_competencias WHERE id = ' + id;

  con.query(sql, function(error, resultado, campos) {
    if(error) {
      console.log("Hubo un error en la consulta para traer una competencia.", error.message);
      return res.status(500).send("Hubo un error en la consulta para traer una competencia.");
    };
    if(!id) {
...
function votar(req, res) {
  ...
  con.query(sql, function(error, resultado, fields) {
    ...
    console.log("Agregaste el voto a la DB.");
  });
};
...
function crearCompetencia(req, res) {
  ...
  var sqlBusqueda = 'SELECT * FROM vs_competencias WHERE nombre = ' + nombre;
  con.query(sqlBusqueda, function(error, resultado, campos) {
    if(resultado && resultado.length !== 0) {
      ...
      con.query(sqlbuscarPeliculas, [nombre], function(error, resultado, campos) {
        if(resultado && resultado.length < 2) {
          console.log("ERROR. La cantidad de resultados obtenidos no es suficiente.");
          return res.status(422).send("ERROR. La cantidad de resultados obtenidos no es suficiente.");
        } else {
...
function eliminarVotos(req, res) {
  ...
  con.query(sql, function(error, resultado, campos) {
    if(error) {
      console.log("No se borraron votos.", error.message);
      return res.status(500).send("No se borraron votos.");
    };
    return res.status(200).send("Se borraron los votos.");
  })
